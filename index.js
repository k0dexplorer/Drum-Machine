...
function App() {
    ...
    return (
        <div id="display" className="min-vh-100 text-white">
            ...
            {recording && (
                <>
                <button onClick={playRecording} className="btn btn-danger">Play</button>
                <button onClick={() => setRecording("")} className="btn btn-outline-info">Clear</button>
...
function Pad ( {clip, volume, setRecording}) {
    ...
    return (
        <div onClick={playSound} id="drum-pad" className={`drum-pad btn btn-gradient p-4 m-3 ${active && 'drum-pad btn-warning'}`}>
